...
void measure (int ledPair, int n, int sensors[], int values_one[], int values_two[]) {
  /* This function turns on an LED pair and measures the given voxels:
   ...
   values_one[] holds the measurements on the first LED (say 940 nm).
   values_two[] holds the measurements on the second LED (say 850 nm).  (values_one and two should be the same size) */
...
void loop() {
  ...
  for(int i = 0; i < 6; i++)
  {
    low_y[i] = log10(low_values[i]) - last_low_y[i];
    high_y[i] = log10(high_values[i]) - last_high_y[i];
    last_low_y[i] = low_y[i];
    last_high_y[i] = high_y[i];
    hb[i] = (1214*low_y[i] - 1058*high_y[i])/d +hb[i]; //we calculate the new delta and add it
    hbO2[i] = (691.32*high_y[i] - 693.44*low_y[i])/d +hbO2[i];
  }
  /* Hb and HbO2 aren't actual values, they are deltas from the initial value. scaled by a constant. */
